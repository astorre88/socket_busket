defmodule SocketBusketWeb.DedupeChannel do
  use Phoenix.Channel
  ...
  def broadcast(numbers, times) do
    Enum.each(1..times, fn _ ->
      Enum.each(numbers, fn number ->
        SocketBusketWeb.Endpoint.broadcast!("dupe", "number", %{number: number})
      end)
    end)
  end

  defp enqueue_send_buffer(%{assigns: %{awaiting_buffer?: true}} = socket), do: socket
